...
function CTA_OnLoad()
	...
	function ChatFrame_OnEvent(event)
		if( CTA_SavedVariables.muteLFGChannel and CTA_MONITOR_CHANNEL_NAME[( arg9 or "?" )] ) then
			return;
		end
		...
		if( CTA_AnnounceTimer > 0 ) then
			...
			if( CTA_AnnounceTimer == 0 and CTA_AutoAnnounce ) then	
				...
				elseif( CTA_LGMPrefixLabel:GetText() and CTA_LFGDescriptionEditBox:GetText() ~= "" ) then
					CTA_SendChatMessage( CTA_LFGDescriptionEditBox:GetText()..CTA_LGMPrefixLabel:GetText(), "CHANNEL", CTA_CHANNEL_ANNOUNCE ); --CTA_AutoAnnounce );
...
function CTA_OnEvent( event ) -- Called by XML on Event
    ...
    --- DEFAULT_CHAT_FRAME:AddMessage("CTA DEBUG: arg9="..tostring(arg9))
		...
		elseif CTA_MONITOR_CHANNEL_NAME[arg9] then -- R10E
...
function CTA_UpdateMinimapIcon()
  ...
  CTA_MinimapMessageFrame:SetPoint("LEFT",CTA_MinimapIcon,"RIGHT",5,10)
