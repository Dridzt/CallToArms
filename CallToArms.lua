...
function CTA_OnLoad()
  ...
  function ChatFrame_OnEvent(event)
    local check = arg9 and string.upper(arg9) or "?"
    if (CTA_SavedVariables.muteLFGChannel and CTA_MONITOR_CHANNEL_NAME[check]) then
      return;
    end
    ...
    if (arg2 and CTA_IgnoreBlacklisted and CTA_FindInList(arg2, CTA_BlackList)) then
      ...
      CTA_LogMsg(CTA_BLOCKED_MESSAGE .. arg2, CTA_BLOCK);
      -- R7
      return;
    end
    ...
    if (not arg1 or strsub(event, 1, 16) ~= "CHAT_MSG_WHISPER" or strsub(arg1, 1, 5) ~= "/cta ") then
      -- not arg1: R3 bug fix
      if (not arg1 or not CTA_MainFrame:IsVisible() or not string.find(arg1, CTA_AWAY_FROM_KEYBOARD)) then
        -- not arg1: R3 bug fix
...
function CTA_SlashHandler(com)
  ...
  if (com == CTA_ANNOUNCE_GROUP .. " on") then
    CTA_AutoAnnounce = CTA_CHANNEL_ANNOUNCE;
    if CTA_AnnounceTimer == 0 then CTA_AnnounceTimer = 5 end;
    -- R10E
    ...
    -- MARKER: THE ELSE PART COULD BE TAKEN OUT NOW
    ...
    if (CTA_AnnounceTimer > 0) then
      ...
      if (CTA_AnnounceTimer == 0 and CTA_AutoAnnounce) then
        ...
        elseif (CTA_MyRaidIsOnline and CTA_MyRaid) then
          ...
          CTA_SendChatMessage(CTA_AcidSummary, "CHANNEL", CTA_CHANNEL_ANNOUNCE);
            ...
            if (pdata) then
              ...
              if (data.whoAttempts < 5) then
                if (CTAWM.getPositionInQueue(name) == 0) then
                  ...
                  CTA_Util.logPrintln("Asking for who info for " .. name .. ", attempt " .. data.whoAttempts);
                  -- R11B5: fixed bug
                  CTAWM.addNameToWhoQueue(name);
                else
                ...
                -- R11B5: fixed bug - was (CTA_WhoName)
              end
            end
          end
        end
...
function CTA_OnEvent(event)
  ...
  if (event == "PLAYER_ENTERING_WORLD" and CTA_Reload) then
  ...
  if (event == "CHAT_MSG_CHANNEL") then
    ...
    --[[ Use channel event to delay joining of CTAChannel - R11 B8: Disabled for now, channel can still be joined manually.
      ...
      for channel in pairs(CTA_MONITOR_CHANNEL_NAME) do
			 CTA_Util.joinChannel( channel );		
      end
    ...
    --- DEFAULT_CHAT_FRAME:AddMessage("CTA DEBUG: arg9="..tostring(arg9))
    local check = arg9 and string.upper(arg9) or "?"
    ...
    elseif CTA_MONITOR_CHANNEL_NAME[check] then
      -- R10E
...
function CTA_ReceiveRaidInvitationRequest(msg, author)
  if (CTA_InvitationRequests[author]) then return; end
  -- cheap short-circuit hack
...
function CTA_BroadcastRaidInfo()
  ...
  if (CTA_MyRaidIsOnline == nil) then return; end
  -- short circuit hack for push changeover in R5
...
function CTA_ApplyFiltersToGroupList()
  ...
  for i = 1, getn(CTA_MessageList) do
    ...
    if (data) then
      ...
      local ok = 1;
      local dhr = chr - tonumber(string.sub(data.time, 1, 2)) or 0;
      -- added: beta 4 hack re: beta 3 reported error
      local dmn = cmn - tonumber(string.sub(data.time, 3, 4)) or 0;
      -- added: beta 4 hack re: beta 3 reported error
      ...
      if (ok) then
        ...
        if (keywords and keywords ~= "*") then
          score = CTA_Util.search(data.message, keywords);
          -- SachaSearch( data.author.." "..data.message, keywords );
        end
        if (score > 0) then
          data.elapsed = dmn;
...
function CTA_MyRaidInstantUpdate()
  ...
  -- MARKER UPDATE NOT COMING OFTEN ENOUGH
...
--]]
  ...
  while (num <= 40) do
    ...
    if (CTA_InvitationRequests[name]) then
      CTA_RemoveRequest(name);
      -- R2: Remove pending member if in raid
...
--]]
  ...
  local pendingSize = 0;
  for name, data in CTA_InvitationRequests do
    if (name and data.status == 2) then
      -- R2: pending members
...
--]]
  ...
  if CTA_MyRaid.minLevel and CTA_MyRaid.minLevel > 1 then
    -- MARKER
    ...
    if (item.classes or item:GetName() == "CTA_Acid0") then
      ...
      if (item:GetName() ~= "CTA_Acid0") then
        ...
        for key, val in item.classes do
          if (nam:GetText()) then
            if (GetLocale() == "deDE") and(key == CTA_WARLOCK) then
              -- R10E German Warlock fix
      ...
      local pval = floor(50 * item.val / CTA_MyRaid.maxSize);
      -- 80 changed to 50 R5
      if (pval > 50) then pval = 50; end
      -- cheapo fix
      ...
      local cval = item.cur;
      local cpval = floor(50 * cval / CTA_MyRaid.maxSize);
      -- 80 -> 50 R5
      if (cpval > 50) then cpval = 50; end
      -- cheapo fix
  ...
  local ExtraOptions = CTA_RP_TYPE
  -- Further options need to be separated with ";"
...
function CTA_ConvertToParty()
  -- R2 (suggested by Sadris) -- Called by XML
...
function CTA_Tab_OnCLick()
  ...
  CTA_ShowLFGButton:SetFrameLevel(CTA_MainFrame:GetFrameLevel() -1);
  -- r7
...
--]]
local quadrants = {
  ["TOPLEFT"] = {"TOPLEFT", "BOTTOMRIGHT", 0, -10},
  ["TOP"] = {"TOP", "TOP", 0, -10},
  ["TOPRIGHT"] = {"TOPRIGHT", "TOPLEFT", 0, -10},
  ["RIGHT"] = {"RIGHT", "LEFT", 0, 0},
  ["BOTTOMRIGHT"] = {"BOTTOMRIGHT", "TOPLEFT", 0, 10},
  ["BOTTOM"] = {"BOTTOM", "BOTTOM", 0, 10},
  ["BOTTOMLEFT"] = {"BOTTOMLEFT", "TOPRIGHT", 0, 10},
  ["LEFT"] = {"LEFT", "RIGHT", 0, 0},
}
function CTA_UpdateMinimapIcon()
  ...
  if icon_y > screen_y/2 then
    quadrant = "TOP"
  elseif icon_y < screen_y/2 then
    quadrant = "BOTTOM"
  else
    quadrant = ""
  end
  if icon_x > screen_x/2 then
    quadrant = quadrant .. "RIGHT"
  elseif icon_x < screen_x/2 then
